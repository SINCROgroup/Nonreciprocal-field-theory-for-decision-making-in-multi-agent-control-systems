function AB_rectangular(params,gamma,delta,DirectoryName)

%%% This function simulates the Agent Based equations in the case
%%% shepherding task is to collect in (if both gamma and delta are >=0) or to
%%% expel from (if both gamma and delta are <=0 )  a rectangular region centred around the origin all the targets.
%%% a circle.

if gamma>=0 && delta >=0

    filename=sprintf("%s/AB_rectangular_containment_g%d_d%d",DirectoryName,round(gamma*10000),round(delta*10000));

elseif gamma<=0 && delta <=0

    filename=sprintf("%s/AB_rectangular_expulsion_g%d_d%d",DirectoryName,round(-gamma*10000),round(-delta*10000));

else
    sprintf("Gamma and delta must be either both positive or negative")
    return
end
...
% herders preferentially chases targets with larger distance from the goal
% (origin)
...
% target (at the back wrt the origin).
...
for it = 1:time_steps
    ...
    % targets and herders polar coordinates needed for computations
    [~, T_theta] = cartesian_to_polar(minimum_image_distance(T(:,1), 0, L, correction), minimum_image_distance(T(:,2), 0, L, correction));
    dH0x = minimum_image_distance(H(:,1), 0, L, correction);
%     dH0y = minimum_image_distance(H(:,2), 0, L, correction);
%     [dH0, ~] = cartesian_to_polar(dH0x, dH0y);
    ...
    if kh ~= 0 && xi ~= 0 %compute interaction/decision-making if interaction strenght and range are non-zero
        ...
        dx_chasing = minimum_image_distance(H(:,1), T(:,1)' + delta*sign(cos(T_theta)'), L, correction);
        dy_chasing = minimum_image_distance(H(:,2), T(:,2)', L, correction);
        IH = attraction(right_norm, dx_chasing, dy_chasing, abs(minimum_image_distance(T(:,1),0,L,correction)), kh, gamma);
