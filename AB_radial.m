function AB_radial(params,gamma,delta,DirectoryName)

%%% This function simulates the Agent Based equations in the case the goal region is
%%% a circle.
...
for it = 1:time_steps
    ...
    if kh ~= 0 && xi ~= 0 %compute interaction/decision-making if interaction strenght and range are non-zero
        ...
        right_norm = right .* exp(-gamma .* dH0); % the matrix indicating whether a given herder has to consider a given target is now multiplied by the exponential weight in the selection rule for each target

        dx_chasing = minimum_image_distance(H(:,1), T(:,1)' + delta .* cos(T_theta)', L, correction);
        dy_chasing = minimum_image_distance(H(:,2), T(:,2)' + delta .* sin(T_theta)', L, correction);
        IH = attraction(right_norm, dx_chasing, dy_chasing, T_rho, kh, gamma);
