function AB_travelling_patterns(params,gamma,delta,DirectoryName)

if gamma*delta<0
    sprintf("delta and gamma must have the same sign (or be 0)")
    return
end
...
for it = 1:time_steps
    ...
    [T_rho, T_theta] = cartesian_to_polar(minimum_image_distance(T(:,1), 0, L, correction), minimum_image_distance(T(:,2), 0, L, correction));
    ...
    if kh ~= 0 && xi ~= 0 %compute interaction/decision-making if interaction strenght and range are non-zero
        ...
        right_norm = right .* exp(-gamma .* dH0x);

        dx_chasing = minimum_image_distance(H(:,1), T(:,1)' + delta*ones(size(T(:,1)')), L, correction);
        dy_chasing = minimum_image_distance(H(:,2), T(:,2)', L, correction);
        IH = attraction(right_norm, dx_chasing, dy_chasing, (minimum_image_distance(T(:,1),0,L,correction)), kh,gamma);
...
filename=sprintf("%s/AB_travelling_patterns_g%d_d%d",DirectoryName,round(gamma*10000),round(delta*10000));
